...
const mailRequestSchema = Joi.object({
...
const MailService: ServiceSchema = {
    ...
    settings: {
        smtpBypass: process.env.SMTP_BYPASS === "true",
    ...
    actions: {
        ...
        sendVerifyRegisterEmail: {
            params: mailRequestSchema as any,
            async handler(ctx: Context<MailRequest>) {
    ...
    async started() {
        ...
        if (this.settings.smtpBypass) {
            this.logger.info("SMTP_BYPASS is enabled. Emails will not be sent.");
            return;
        }
